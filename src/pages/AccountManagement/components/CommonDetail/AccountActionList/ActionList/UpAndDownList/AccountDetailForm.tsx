...
const AccountDetailList: FC<{ record: OnlineAndOfflineListItem }> = ({
    record,
}) => {
    ...
    const accountListColumns: ProColumns<CreditDetailListItem>[] = [
        ...
        },
        {
            title: '未逾期金额(万)',
            render: (text, record, index, action) => {
                const result = new Big(record.signed_amount ?? 0)
                    .sub(record.overdue_amount ?? 0)
                    .toNumber();
                return <Currency value={result < 0 ? 0 : result}></Currency>;
            },
        },
    ];
...
const AccountDetailForm: FC<AccountDetailFormProps> = ({ record }) => {
    return (
        <ModalForm
            title="户口详情"
        ...
        >
            ...
            <AccountDetailList record={record}></AccountDetailList>
        </ModalForm>
    );
};
